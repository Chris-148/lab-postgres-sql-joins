...
```sql
SELECT books.*, authors.* FROM books FULL JOIN authors ON books.author_id = authors.id;
```
...
```sql
SELECT publishers.name AS publisher_name, publishers.location, books.title AS book_title FROM publishers LEFT JOIN books ON publishers.id = books.publisher_id;
```
...
```sql
SELECT books.title, publishers.name, publishers.location FROM publishers RIGHT JOIN books ON publishers.id = books.publisher_id;
```
...
```sql
SELECT authors.*, books.*, publishers.* FROM authors 
